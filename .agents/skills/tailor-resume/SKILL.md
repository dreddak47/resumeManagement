...
## Preconditions
If a `jd-analyze` brief hasn't been produced yet for this JD in the current conversation, run that analysis first (see `.Codex/skills/jd-analyze/SKILL.md`) before writing any resume file.
...
8. **Run a review pass.** After generating the tailored resume, invoke the `resume-fact-checker` agent (`.Codex/agents/resume-fact-checker.md`) to cross-check every claim against the context store before presenting the final file to the user.
